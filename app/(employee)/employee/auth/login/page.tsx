...
export default function EmployeeLoginPage() {
  ...
  // Use the auth-helpers client for proper cookie-based session persistence
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/secure-employee-signin`, {
        ...
        body: JSON.stringify({ email, password, configId: '97f8bdb8-b550-4b71-b9e7-71938ecd3393' }),
      });
      ...
      if (data.status === 'otp_sent') {
        ...
        if (session) {
          ...
          router.push('/employee'); // Redirect to employee dashboard
        } else {
  ...
  return (
    ...
    >
      <h2 className="text-2xl font-bold mb-4">{showOtpInput ? 'Enter OTP' : 'Employee Login'}</h2>
      {!showOtpInput ? (
      ...
      {/* No signup link for employees in this portal, assuming accounts are provisioned */}
    </motion.div>
  );
}
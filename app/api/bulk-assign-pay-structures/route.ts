...
import { supabase } from '@/lib/supabase'; // Use the standard client

export async function POST(request: Request) {
  const { params, accessToken } = await request.json();

  if (!params || !accessToken) {
    return NextResponse.json({ success: false, message: 'Missing parameters or accessToken' }, { status: 400 });
  }
  ...
  try {
    ...
    const { data, error } = await supabase.functions.invoke('a_crud_universal_pg_function_gateway', {
      body: {
        config_id: 'assign-employee-pay-structures',
        params: {
          assignments: params // The PG function expects the data under an 'assignments' key
        },
      },

...
export default function PayrollAdminLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/secure-employee-signin`, {
        ...
        body: JSON.stringify({ email, password, configId: 'b7e2e76a-090f-4584-bf34-8495a1a63812' }),
      });
      ...
      if (data.status === 'otp_sent') {
        ...
        if (session) {
          ...
          router.push('/payroll_admin'); // Redirect to payroll_admin dashboard
        } else {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      ...
      >
        <h2 className="text-2xl font-bold mb-4">{showOtpInput ? 'Enter OTP' : 'Payroll Admin Login'}</h2>
        {!showOtpInput ? (

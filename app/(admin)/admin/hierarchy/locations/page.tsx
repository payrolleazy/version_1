...
export default function LocationMasterPage() {
  ...
  const [metadataItem, setMetadataItem] = useState<Location | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const configId = activeTab === 'approved'
        ...
        : HMS_GATEWAY_CONFIGS.LOCATION_READ_PENDING;
  ...
  const handleOpenFormModal = (item?: Location) => {
    setEditingItem(item || null);
    setIsFormModalOpen(true);
  };

  const handleOpenConfirmModal = (action: 'approve' | 'reject', items: Location[]) => {
  ...
  // Metadata Handler
  const handleViewMetadata = (item: Location) => {
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      const res = await fetch(API_ENDPOINTS.EXCEL_TEMPLATE, {
        ...
        body: JSON.stringify({
          config_id: HMS_GATEWAY_CONFIGS.LOCATION_TEMPLATE_DOWNLOAD,
          params: {},
  ...
  return (
    <ErrorBoundary>
      ...
      {isBulkUploadModalOpen && (
        <Modal isOpen={isBulkUploadModalOpen} onClose={() => setIsBulkUploadModalOpen(false)} title="Bulk Upload Locations">
          <BulkUpload
            config_id={HMS_GATEWAY_CONFIGS.LOCATION_EXCEL_UPLOAD}
      ...
      {isExportModalOpen && (
        <ExportUserRolesModal
          ...
          config_id={HMS_GATEWAY_CONFIGS.LOCATION_EXPORT_EXCEL}
          title="Export Locations to Excel"
        />
      )}
      ...
      {isConfirmModalOpen && (
        <Modal
          ...
          title={`${actionType === 'approve' ? 'Approve' : 'Reject'} Location(s)`}
        >
          <div className="space-y-4">
            <p className="text-gray-600">
              Are you sure you want to {actionType}{' '}
              <strong>{selectedForAction.length}</strong> selected location(s)?
            </p>
            <ul className="text-sm text-gray-700 bg-gray-50 rounded p-3 max-h-40 overflow-y-auto space-y-1">
              {selectedForAction.map(b => (
                <li key={b.id}>{b.location_name}</li>
              ))}
            </ul>
        ...
        >
          <div className="space-y-4">
            <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
              <h3 className="text-sm font-bold text-gray-700 mb-3 border-b pb-2">{metadataItem.location_name} Details</h3>
...
function LocationFormModal({ isOpen, onClose, onSuccess, item, schema }: FormModalProps) {
  ...
  useEffect(() => {
    if (item) {
      ...
      const empty: Record<string, any> = { location_name: '' };
      schema.forEach(field => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await callPgFunction(
        HMS_GATEWAY_CONFIGS.LOCATION_BULK_INSERT,
        { input_rows: [payload] },
        session.access_token
      );
